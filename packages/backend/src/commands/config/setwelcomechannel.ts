...
const cmd: Command = {
    ...
    execute: async (msg, args) => {
        ...
        if (args[0] && args[0].startsWith("<")) {
            ...
            channel = await msg.guild.channels.cache.find(value => value.id == channelId) as TextChannel
            if (!channel) return
        ...
        if(channel) {
            ...
            await msg.reply("Succesfully changed welcome channel to #" + channel.name)
